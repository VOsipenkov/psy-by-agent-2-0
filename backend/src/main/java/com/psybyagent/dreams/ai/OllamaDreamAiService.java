...
public class OllamaDreamAiService implements DreamAiService {
    ...
    private static final Set<String> STOP_WORDS = Set.of(
        "и", "в", "во", "на", "но", "что", "как", "мне", "мен€", "был", "была", "были",
        "это", "этот", "она", "они", "его", "ее", "мы", "вы", "ты", "€", "сон", "сна",
        "потом", "когда", "где", "или", "под", "над", "из", "за", "дл€", "очень"
    );
    ...
    private DreamAiResult toResult(OllamaAnalysisPayload payload, DreamConversation conversation) {
        ...
        String assistantMessage = StringUtils.hasText(payload.assistantMessage())
            ? payload.assistantMessage().trim()
            : "„то в этом сне запомнилось вам сильнее всего: место, человек или чувство?";
    ...
    private DreamAiResult fallback(DreamConversation conversation) {
        ...
        if (userMessages.size() < 2) {
            String assistantMessage = "ћне нужно еще немного контекста.  акие чувства вы испытывали во сне и какой образ или символ запомнилс€ сильнее всего?";
        ...
        String interpretation = "ѕо мотивам сонника ћиллера в этом сне выдел€ютс€ символы %s. “акой сюжет обычно св€зан с внутренним напр€жением, попыткой что-то осмыслить и потребностью вернуть себе ощущение опоры. —тоит обратить внимание на эмоции из сна и на то, с чем они перекликаютс€ в вашей текущей жизни."
